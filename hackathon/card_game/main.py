...
def main():  # khó
    ...
    while option != 8:
        ...
        if option == 2:
            if user.flag == True:
                print("Đang trong ván rồi nhé!")
            else:
                user.add_player()
        if option == 3:
            if user.flag == True:
                print("Đang trong ván rồi, không xóa được đâu")
            else:
                user.remove_player()
        if option == 4:
            if user.flag == True:
        ...
        if option == 5:
            if user.flag == False:
                print("Cần chia bài trước khi lật")
            else:
                for i in user.number:
                    print(f'Tên người chơi: {i.name} Bộ bài: {i.flip_card()} Điểm: {i.point}')
                print("Người chơi thắng là: " +user.flip_card())    
